...
import './Firstyearlist.css'; //same css sang Firstyear
import axios from 'axios';
import { useNavigate, Link } from 'react-router-dom';

const BSITSecondyearlist = () => {
      ...
      const fetchStudents = async () => {
        try {
          ...
          const firstYearBSITStudents = response.data.filter(student => {
            ...
            return student.yearlevel === "2nd Year" && student.course === "BSIT";
          });
        ...
        <div className="firstyearlist-leftcont-box">
          ...
          <div className="firstyearlist-backevent-btnbox">
            <div className="firstyearlist-order1">
              ...
              <button className="firstyearlist-backevent-btn" onClick={goToYearLevel}>Back to Year Levels</button>
            </div>
            <h4 className="firstyearlist-backbtnh4">Press to return to the year levels page</h4>
          </div>
  
          <div className="firstyearlist-addeventForm">
            <h2 className="firstyearlist-addeventformH2">Bachelor of Science in Information Technology</h2>
            <div className="firstyearlist-eventDetails-box">
              <h4 className="firstyearlist-addeventformH4">(BSIT) Second Year</h4>
            </div>
                ...
                <tbody>
                  {students.map((data, index) => (
                    <tr key={index}>
                      ...
                      <td>{data.skill}</td>
                      <td>
                        <Link to={`updatesecondyear/${data.id}`} className='firstyearlist-updatebtn'>Update</Link>
...
export default BSITSecondyearlist